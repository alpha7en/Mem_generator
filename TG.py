...
async def process_callback_c(callback_query: types.CallbackQuery):
    ...
    if code == '2':
        ...
        os.remove(str(callback_query.from_user.id) + '.jpg')
        await bot.delete_message(callback_query.from_user.id, int(db.get_args(callback_query.from_user.id)[3]))
        await bot.send_message(callback_query.from_user.id, 'это умора однозначно, а тебе как?', reply_markup=kb.greet_kb)
        db.obr_set(callback_query.from_user.id, 1)
    elif code == '8':
        ...
        os.remove(str(callback_query.from_user.id) + '.jpg')
        await bot.delete_message(callback_query.from_user.id, int(db.get_args(callback_query.from_user.id)[3]))
        await bot.send_message(callback_query.from_user.id, 'бот хорош, а тебе как?',
        ...
        os.remove(str(callback_query.from_user.id) + '.jpg')
        await bot.delete_message(callback_query.from_user.id, int(db.get_args(callback_query.from_user.id)[3]))
        await bot.send_message(callback_query.from_user.id, 'как по мне достойно, а тебе как?',
                               reply_markup=kb.greet_kb)
        db.obr_set(callback_query.from_user.id, 1)
    elif code == '13':
        flags.fl(str(callback_query.from_user.id) + '.jpg')
        f = open(str(callback_query.from_user.id) + '.jpg', "rb")
        await bot.send_photo(callback_query.from_user.id, photo=f)
        await bot.answer_callback_query(callback_query.id)
        os.remove(str(callback_query.from_user.id) + '.jpg')
        await bot.delete_message(callback_query.from_user.id, int(db.get_args(callback_query.from_user.id)[3]))
        await bot.send_message(callback_query.from_user.id, 'мне кажеться шикарно, а тебе как?',
                               reply_markup=kb.greet_kb)
        db.obr_set(callback_query.from_user.id, 1)
